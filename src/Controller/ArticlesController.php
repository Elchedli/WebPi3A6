...
class ArticlesController extends AbstractController
{
     ...
     * @return Response
     * @Route("/stat_art", name="statistique_Art")
     */
 public function statistique(ArticleRepository $articleRepository)
    {
        $articles = $articleRepository->findAll();
        $catType= ['arts', 'Méditation', 'Nourriture','santé','sport']; #, 'Suivi', 'Msg', 'Technical', 'Posts'];
        $catColor = ['#49A9EA', '#36CAAB', '#34495E','#ff0080','#ffff00'];
        $catArts= count($articleRepository->findBy(["idCat" =>"61"]) )  ;
        $catSante = count($articleRepository->findBy(["idCat" =>"27"]) ) ;
        $catMed = count($articleRepository->findBy(["idCat" => "6"]) ) ;
        $catMedi = count($articleRepository->findBy(["idCat" => "23"]) ) ;
        $catMedita = count($articleRepository->findBy(["idCat" => "5"]) ) ;


        $catCount = [ $catArts, $catSante,$catMed,$catMedi,$catMedita]; #,$categSuivi, $categMsg, $categTechnical, $categPosts];
    ...
    /**
     * @Route("/pdf/yasmine/{idArt}", name="imprimer_y", methods={"GET"})
     */
    ...
    public function new(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $article->setAuteurArt($this->session->get('user'));
